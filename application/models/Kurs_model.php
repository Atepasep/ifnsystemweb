...
{
   public function getdatarate($filtkat, $mode){
        // $query = $this->getdata($mode);
        // $cari = array('barang.kode','nama_barang','nama_kategori');
        // $query = "Select * from tb_kurs_bi order by period desc";
        $this->db->select("*,MONTHNAME(period) as bulan, YEAR(period) as tahun,concat(YEAR(period),' - ',MONTHNAME(period)) as xtahun");
        $this->db->from('tb_kurs_bi');
        $this->db->order_by('period desc');
        $dataquery = $this->db->get_compiled_select();
        $query = "Select * from (".$dataquery.") r1";
        $cari = array('period','bulan','tahun','xtahun');
        $where = $filtkat;
        $isWhere = null;
   ...
   public function getdatakmk($filtkat, $mode){
        // $query = $this->getdata($mode);
        // $cari = array('barang.kode','nama_barang','nama_kategori');
        // $query = "Select * from tb_kurs_bi order by period desc";
        $this->db->select("*");
